...
const  antibioticsDrugs = drugs.filter(function(drug){ return drug.category === "Antibiotic"})
console.log(antibioticsDrugs);
// ---------------------------------------------------------------------------------------------------------------------------------------------------------------
// // 2. Return an array of drug names in lowercase.
    const drugNamesInLowerCase = drugs.map((drug) => {return drug.name.toLowerCase()})
    console.log(drugNamesInLowerCase)
// ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
// // 3. Write a function that accepts a category and returns all drugs under that category.
const drugsByCategory = (Category) => { return drugs.filter(drug => {return drug.category === Category})};

// // function test calls
console.log(drugsByCategory("Supplement"));
console.log(drugsByCategory("Antacid"));
...
const drugsWithPrescription = drugs.filter((drug)=> {return drug.isPrescriptionOnly === true})
console.log(drugsWithPrescription)
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------
// 6. Return a new array, each item should follow the format: "Drug: [name] - [dosageMg]mg".
const newArray = drugs.map((drug) => {return `Drug: ${drug.name} - ${drug.dosageMg}mg`}) 
...
const noPrescriptionDrugs = drugs.filter((drug) => {return (drug.isPrescriptionOnly !== true)})
...
const drugStockByManufacturer = (manufacturingCompany) => {return drugs.filter((drug) => {return drug.manufacturer === manufacturingCompany})}

// // function  call tests
console.log(drugStockByManufacturer("Teva"))
console.log(drugStockByManufacturer("Roche"))
console.log(drugStockByManufacturer("Pfizer"))

// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
// 10. Use forEach() to count how many drugs are Analgesics.
let countOfAnalgesicDrug = 0;
drugs.forEach((drug) => {if (drug.category === "Analgesic"){countOfAnalgesicDrug += 1}})
console.log(countOfAnalgesicDrug);
